      ...
      >
        ...
        <form onSubmit={handleSearch} className="mb-8">
          <div className="flex gap-3">
            <input
              ...
              className="flex-1 rounded-xl border border-white/10 bg-[var(--color-bg-card)] px-4 py-3 text-[var(--color-text)] placeholder:text-[var(--color-text-muted)]/60 focus:outline-none focus:ring-2 focus:ring-amber-500/50"
              disabled={loading}
            />
            <button
              type="submit"
              disabled={loading || !query.trim()}
              className="px-6 py-3 rounded-xl bg-amber-500 text-[var(--color-bg)] font-semibold hover:bg-amber-400 disabled:opacity-50 transition-colors"
            >
        ...
        {result && (
          <motion.div
            initial={{ opacity: 0, y: 12 }}
            animate={{ opacity: 1, y: 0 }}
            className="rounded-2xl border border-white/10 bg-[var(--color-bg-card)] p-6"
          >
            <div className="flex items-center justify-between mb-4">
              <span className="inline-flex px-3 py-1 rounded-lg bg-amber-500/20 text-amber-400 text-sm font-medium">
                {result.level}
              </span>
              {result.type === 'vocab' && (
                <button className="text-sm font-medium text-amber-400 hover:text-amber-300">
