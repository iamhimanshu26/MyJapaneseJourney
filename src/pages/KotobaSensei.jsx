      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5">
          ...
          <textarea
            ...
            className="h-28 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-800"
          />
          <ActionButton type="button" onClick={askSensei} disabled={loading || !question.trim()} variant="primary" className="mt-3">
            Ask Kotoba Sensei
          </ActionButton>
        </div>
        ...
        {response ? (
          <section className="mt-4 rounded-xl border border-slate-200 bg-white p-5">
            <h3 className="text-base font-medium text-slate-900">Sensei Response</h3>
            <p className="mt-3 text-sm text-slate-700">{response.answer}</p>
            <p className="mt-2 text-xs text-slate-500">Confidence: {Math.round(Number(response.confidence || 0) * 100)}%</p>
            <div className="mt-4 space-y-2">
              {(response.recommendations || []).map((item) => (
                <article key={`${item.title}-${item.action}`} className="rounded-lg border border-slate-200 bg-slate-50 p-3">
                  <p className="text-sm font-medium text-slate-900">{item.title}</p>
                  <p className="text-xs text-slate-500">{item.detail}</p>
                  <Link to={item.action || '/'} className="mt-1 inline-block text-xs text-blue-600 hover:underline">
                    Open module
                  </Link>
                </article>
              ))}
        ...
        {history.length ? (
          <section className="mt-4 rounded-xl border border-slate-200 bg-white p-5">
            <h3 className="text-base font-medium text-slate-900">Recent Copilot Sessions</h3>
            <div className="mt-3 space-y-2">
              {history.map((item, idx) => (
                <button
                  ...
                  className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-left hover:border-blue-400"
                >
                  <p className="text-sm text-slate-900">{item.question}</p>
                  <p className="line-clamp-1 text-xs text-slate-500">{item.response?.answer}</p>
                </button>
              ))}
            </div>
          </section>
